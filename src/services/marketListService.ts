...
// Remove um item da lista pelo ID dele
...
export const clearList = async (whatsappId: string) => {
  const userId = await getUserId(whatsappId);
  await pool.query("DELETE FROM market_list_items WHERE user_id = $1", [
    userId,
  ]);
  return { message: "Lista de compras limpa." };
};
...
export const getList = async (whatsappId: string) => {
  const userId = await getUserId(whatsappId);
  const res = await pool.query(
    "SELECT * FROM market_list_items WHERE user_id = $1 ORDER BY created_at ASC",
    [userId]
  );
  return res.rows;
};
